...
const Projectlist: React.FC = () => {
  ...
  const fetchProjects = () => {
    ...
    fetchInstance
      .get<{ status: String; data: Record<string, ProjectResponse> }>(
        `/projects/${memberId}`
      )
      .then((response) => {
        const projectList = Object.entries(response.data.data).map(
  ...
  const fetchIssues = async (projectId: number) => {
    try {
      ...
      if (issues.some((issue: Issue) => !issue.issueId)) {
        throw new Error(
          `Some issues in project ${projectId} do not have an issue ID.`
        );
      }
  ...
  return (
    <div className={style.div}>
      <div className={style.textcontainer}>
        ...
        <p className={style.projectLink} onClick={() => navigate("/project")}>
          모두보기
        </p>
      </div>
        ...
        <div className={style.projectlistdiv}>
          {projects.map((project) => (
            <div className={style.projectdiv}>
              <span className={style.projectname} key={project.projectId}>
                {project.projectName}
              </span>
                ...
                <div>
                  {projectIssues[project.projectId].map((issue) => (
                    <div className={style.epic}>
                      ...
                      />
                      <span key={issue.issueId} className={style.epicname}>
                        {issue.issueTitle}
                      </span>
                    </div>
                  ))}
